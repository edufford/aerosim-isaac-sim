...
The patch strings were verified against repo_build 1.13.2 but are designed to be
version-agnostic: each patch checks whether the old pattern is present (applies it),
the new pattern is already present (skips it), or neither (warns).
"""
...
def main():
    if not os.path.isdir(REPO_BUILD_DIR):
        ...
        print("       Run 'tools\\packman\\packman.cmd pull tools\\deps\\repo-deps.packman.xml' first.")
        sys.exit(1)
